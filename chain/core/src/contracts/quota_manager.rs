...
const METHOD_NAME: &'static [u8] = &*b"getUsersQuota()";
...
lazy_static! {
    ...
    static ref USERS_METHOD_HASH: Vec<u8> = {
        let out :&mut[u8;32] = &mut [0;32];
        let outptr = out.as_mut_ptr();
        unsafe {
            sha3_256(outptr, 32, USERS_METHOD_NAME.as_ptr(), USERS_METHOD_NAME.len());
        }
        let func = out[0..4].to_vec();
        func
    };
...
pub type Quota = String;

pub struct QuotaManager;

impl QuotaManager {
    pub fn read(chain: &Chain) -> Quota {
        ...
        let s = ToHex::to_hex(output.as_slice());
        info!("quota is {:?}", s);
    ...
    pub fn users(chain: &Chain) -> Vec<Address> {
        let call_request = CallRequest {
            from: None,
            to: *CONTRACT_ADDRESS,
            data: Some(USERS_METHOD_HASH.to_vec()),
        };

        trace!("data: {:?}", call_request.data);
        let output = chain.eth_call(call_request, BlockId::Latest).unwrap();
        info!("users output: {:?}", output);
        info!("users: {:?}", ToHex::to_hex(output.as_slice()));
        let users: Vec<Address> = parse_string_to_addresses(&output);
        users
    }
}
